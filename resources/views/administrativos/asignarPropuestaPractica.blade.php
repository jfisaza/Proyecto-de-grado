...
<article class="logo"height="70px" width="110px">
    ...
    <div id="pagina">
        <div id="formulario">
            ...
            <form method="post" action="{{action('AdministrativosController@asignarCalificadorPropuestaPractica')}}" role="form" class="form" enctype="multipart/form-data">
                {{ csrf_field() }}
                
                <div class="form-group">
                    
                    <strong>Código: </strong><p>{{ $propuesta->pp_id }}</p>
                    <strong>Titulo: </strong><p>{{ $propuesta->pp_titulo }}</p>
                    
                </div>
                <input type="hidden" name="pp_id" value="{{ $propuesta->pp_id }}">
