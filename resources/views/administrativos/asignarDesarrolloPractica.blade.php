...
<article class="logo"height="70px" width="110px">
    ...
    <div id="pagina">
        <div id="formulario">
            ...
            <form method="post" action="{{action('AdministrativosController@asignarCalificadorDesarrolloPractica')}}" role="form" class="form" enctype="multipart/form-data">
                {{ csrf_field() }}
                
                <div class="form-group">
                    
                    <strong>Código: </strong><p>{{ $desarrollo->dp_id }}</p>
                    <strong>Titulo: </strong><p>{{ $desarrollo->dp_titulo }}</p>
                    
                </div>
                <input type="hidden" name="dp_id" value="{{ $desarrollo->dp_id }}">
